...
{
	assert(vertices != nullptr && "ѕараметр vertices конструктора Model не может иметь значение nullptr");
	assert(faces != nullptr && "ѕараметр faces конструктора Model не может иметь значение nullptr");
	assert(normals != nullptr && "ѕараметр normals конструктора Model не может иметь значение nullptr");
	assert(tex_coords != nullptr && "ѕараметр tex_coords конструктора Model не может иметь значение nullptr");
...
Model::Model(const char *filename)
{
	...
	file.open(filename);
	if (not file)
		throw std::runtime_error("Ќе могу открыть файл модели");
