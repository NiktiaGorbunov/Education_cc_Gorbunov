 ...
 * У сотрудников есть ФИО, должность, зарплата.
...
void change_employee(std::vector <Employee> &list){
	...
	if ( n < 0 || n > list.size()){
		std::cout << "Такого номера нет в базе!" << std::endl;
	}else{
...
void delte_employee(std::vector <Employee> &list){
...
/*
 *	удаление записи
 *	empl_roster.erase(empl_roster.begin() + 0);\
 *	Начало + номер записи
 */
...
menu:
		...
		if(not(std::cin >> choice)){
			if(std::cin.eof()){
				std::cerr << "ошбика вводы-вывода" << std::endl;
				return 1;
			}
			std::cin.clear();
			std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
