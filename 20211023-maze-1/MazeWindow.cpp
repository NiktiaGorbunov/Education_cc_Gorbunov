...
:Window(WINDOW_WIDTH, WINDOW_HEIGHT)
{
	...
	SDL_SetWindowTitle(_window.get(), "MazeWindow");
...
void MazeWindow::draw_view()
{
	...
	SDL_Rect sky_rect {0, 0, getWidth(), getHeight()/2};
	SDL_Rect floor_rect {0 , getHeight()/2, getWidth(), getHeight()/2 };
	...
	for (int col = 0; col < getWidth(); ++col){
		...
		int h;
		int ctv, cth, ct = 1;
		gamma = atan2(double(col - getWidth()/2), Ds);
		beta = alpha + gamma;
